...
class AuthController(QObject):
    ...
    def register(self, username: str, password: str, email: str) -> str | None:
        """
        ...
        try:
            ...
            if bcrypt.checkpw(password.encode("utf-8"), password_hash):
                self.user_model.set_current_user(
                    user_id=user_data["id"], username=user_data["username"], email=user_data["email"]
                )
    ...
    def update_user_profile(self, user_id: int, username: str | None = None, email: str | None = None) -> bool:
        """
        ...
        try:
            ...
            if username is not None and not self._validate_username(username):
                logger.warning(f"Profile update failed: Invalid username format: {username}")
                return False

            if email is not None and not self._validate_email(email):
