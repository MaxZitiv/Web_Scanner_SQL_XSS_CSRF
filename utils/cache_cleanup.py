...
class CacheCleanupManager:
    ...
    def __init__(self):
        self.cleanup_stats: dict[str, dict[str, bool | int]] = {
        ...
        self.cleanup_start_time: float | None = None
        self.cleanup_end_time: float | None = None
    ...
    def get_cache_sizes(self) -> dict[str, int]:
        ...
        try:
            # Размер кэша сканера
            from scanner.scanner_fixed import DNS_CACHE, FORM_HASH_CACHE, HTML_CACHE, URL_PROCESSING_CACHE

            scanner_cache_size: int = (
                len(HTML_CACHE) + len(DNS_CACHE) + len(FORM_HASH_CACHE) + len(URL_PROCESSING_CACHE)
            )
    ...
    def clear_user_cache(self) -> bool:
        ...
        try:
            ...
            self.cleanup_stats["user_cache"] = {"cleared": True, "size_before": size_before, "size_after": size_after}
    ...
    def clear_error_cache(self) -> bool:
        ...
        try:
            ...
            self.cleanup_stats["error_cache"] = {"cleared": True, "size_before": size_before, "size_after": size_after}
    ...
    def perform_memory_cleanup(self) -> bool:
        ...
        try:
            memory_before: dict[str, int | float] = self.get_memory_usage()
            ...
            memory_after: dict[str, int | float] = self.get_memory_usage()
    ...
    def cleanup_all_caches(self, safe_mode: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            initial_memory: dict[str, int | float] = self.get_memory_usage()
            ...
            logger.info(
                f"Cache cleanup completed in {duration:.3f}s: "
                f"{total_entries_freed} entries, {memory_freed_mb:.2f}MB memory"
            )
            return result
