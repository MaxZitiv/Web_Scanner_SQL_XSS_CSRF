...
def get_security_cache_stats() -> tuple[int, dict[str, Any]]:
    ...
    stats = {"ttl": _cache_ttl, "active_entries": len(_validation_cache), "expired_entries": len(expired_keys)}
    return len(_validation_cache), stats


# Известные опасные домены и паттерны
DANGEROUS_DOMAINS = {"localhost", "127.0.0.1", "::1", "255.255.255.255", "example.com", "test.com", "invalid.com"}
...
def is_bind_address_safe(bind_address: str, config: dict[str, Any] | None = None) -> tuple[bool, str]:
    """
...
def _get_cached_result(key: str) -> bool | None:
    ...
    try:
        ...
        for param, values in query_params.items():
            for value in values:
                ...
                if any(
                    xss_pattern in value.lower() for xss_pattern in ["<script", "javascript:", "onerror=", "onload="]
                ):
...
def rate_limit_check(
    identifier: str, attempts: dict[str, list[float]], max_attempts: int = 5, window_seconds: int = 300
) -> bool:
    """
