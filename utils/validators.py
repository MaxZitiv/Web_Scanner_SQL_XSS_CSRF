...
class DataValidator:
    ...
    def validate_domain(self, domain: str) -> bool:
        ...
        return all(not (len(part) > 63 or len(part) == 0) for part in parts)
    ...
    def validate_scan_config(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if "max_depth" in config and (not isinstance(config["max_depth"], int) or config["max_depth"] < 1):
            issues.append("max_depth must be a positive integer")
            valid = False

        if "max_concurrent" in config and (
            not isinstance(config["max_concurrent"], int) or config["max_concurrent"] < 1
        ):
