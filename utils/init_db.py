...
def migrate_database():
    ...
    try:
        ...
        new_scan_columns = [("scan_type", 'TEXT NOT NULL DEFAULT "general"'), ("status", 'TEXT DEFAULT "completed"')]
...
def check_database_integrity():
    ...
    try:
        ...
        required_users_columns = ["id", "username", "email", "password_hash", "created_at"]
        optional_users_columns = ["last_login", "failed_attempts", "locked_until"]
        required_scans_columns = ["id", "user_id", "url", "result", "timestamp"]
        optional_scans_columns = ["scan_type", "status"]
        ...
        if users_required_ok and scans_required_ok:
            ...
            logger.warning(
                ...
                f"Missing required users columns: {missing_users}. "
                f"Missing required scans columns: {missing_scans}."
            )
            return False
