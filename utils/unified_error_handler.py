...
class UnifiedErrorHandler:
    ...
    def show_warning_message(self, title: str, message: str):
        log_and_notify("warning", f"{title}: {message}")

    def show_info_message(self, title: str, message: str):
        log_and_notify("info", f"{title}: {message}")
    ...
    def _retry_strategy(self, context: str, attempt: int) -> bool:
        if attempt < self.max_retries:
            delay = min(2**attempt, 10)
            logger.info(f"Retrying {context} after {delay}s (attempt {attempt + 1})")
            time.sleep(delay)
            return True
        log_and_notify("error", f"Max retries exceeded for {context}")
        return False

    def _log_and_continue(self, exc: Exception, context: str, attempt: int) -> bool:
        logger.warning(f"Continuing after {type(exc).__name__} in {context} (attempt {attempt + 1})")
        return True

    def _create_file_if_missing(self, exc: Exception, context: str) -> bool:
        import os
        ...
        if "No such file" in str(exc):
            try:
                ...
                log_and_notify("error", f"Failed to create missing file in context {context}: {e}")
        return False

    def _log_permission_error(self, exc: Exception, context: str) -> bool:
        log_and_notify("error", f"Permission denied in {context}: {exc}")
        return False
    ...
    def handle_exception(self, exc: Exception, context: str = "", attempt: int = 0) -> bool:
        ...
        log_and_notify("error", f"Exception in {context}: {exc_type}: {exc}")

        strategy = self.recovery_strategies.get(exc_type, self._default_strategy)
        try:
            return strategy(exc, context, attempt)
        except Exception as e:
            log_and_notify("error", f"Error in recovery strategy for {exc_type}: {e}")
            return False

    def _default_strategy(self, exc: Exception, context: str) -> bool:
        log_and_notify("error", f"Unhandled exception type {type(exc).__name__} in {context}")
        return False
    ...
    def retry_on_exception(
        self, max_retries: int = 3, exceptions: tuple[type[Exception], ...] = (Exception,)
    ...
    def handle_general_error(self, error: Exception, context: str = "") -> bool:
        ...
        log_and_notify("error", f"General error in {context}: {error!s}")
