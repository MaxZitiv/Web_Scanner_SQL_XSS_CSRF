...
class ExportUtils:
    ...
    @staticmethod
    def export_data(
        parent_widget: QWidget, scans: list[dict[str, Any]], format_name: str, file_extension: str, user_id: int
    ) -> bool:
        """
        ...
        path, _ = QFileDialog.getSaveFileName(
            parent_widget, "Сохранить отчет", default_filename, f"{format_name} Files (*.{file_extension})"
        )
        ...
        try:
            ...
            if export_func(scans, path, user_id):
                error_handler.show_info_message("Экспорт завершён", f"Файл {format_name} успешно сохранён.")
                return True
            else:
                error_handler.show_error_message("Ошибка экспорта", f"Не удалось сохранить файл {format_name}.")
                return False
