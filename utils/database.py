...
encrypt_sensitive_data: Callable[[str | dict[str, Any] | list[Any]], str] = encrypt_sensitive_data
decrypt_sensitive_data: Callable[[str], str | dict[str, Any] | list[Any]] = decrypt_sensitive_data
decrypt_sensitive_data_safe: Callable[[Any, Any], str | dict[str, Any] | list[Any] | Any] = decrypt_sensitive_data_safe
...
class Database:
    ...
    def get_db_connection(self) -> sqlite3.Connection:
        ...
        if self._connection is not None:
            try:
                ...
                cur.fetchone()
            except (sqlite3.ProgrammingError, sqlite3.OperationalError, sqlite3.DatabaseError):  # fmt: skip
        ...
        try:
            with self.get_db_connection_cm() as conn:
                ...
                for column, ddl in (
                    ...
                    # Колонка уже может существовать — ошибка ожидаемая и не критичная.
    ...
    def safe_execute(self, query: str, params: tuple[Any, ...] | None = None) -> list[dict[str, Any]]:
        """
    ...
    def get_user_by_username_or_email(self, identifier: str) -> dict[str, Any] | None:
        ...
        try:
            with self.get_db_connection_cm() as conn:
                ...
                if user_data:
                    ...
                    columns = [column[0] for column in cursor.description]
                    user_dict = dict(zip(columns, user_data, strict=False))
    ...
    def get_all_users(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            with self.get_db_connection_cm() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT id, username, email, created_at, last_login FROM users ORDER BY created_at DESC LIMIT ?",
                    (limit,),
                )
                ...
                for vuln in results:
                    if "type" in vuln and ("details" in vuln or "description" in vuln):
                        ...
                        if isinstance(raw_details, dict):
                            full_details = vulnerability_dict_to_details(cast(dict[str, Any], raw_details))
    ...
    def get_scan_by_id(self, scan_id: int, user_id: int) -> dict[str, Any] | None:
    ...
    def get_all_vulnerabilities(self, user_id: int | None = None) -> list[dict[str, Any]]:
    ...
    def get_vulnerability_by_id(self, vuln_id: int) -> dict[str, Any] | None:
        ...
        try:
            with self.get_db_connection_cm() as conn:
                ...
                comments: list[dict[str, Any]] = (
                    cast(list[dict[str, Any]], comments_value) if isinstance(comments_value, list) else []
                )
