...
def format_scan_data_for_export(scans: list[dict[str, Any]], user_id: int | None = None) -> list[dict[str, Any]]:
    ...
    for scan in scans:
        ...
        is_vulnerability_row = (not results) and any(
        ...
        if is_vulnerability_row:
            ...
            vuln_row: dict[str, Any] = {
                ...
                "status": str(
                    scan.get("status", "Vulnerable" if severity.lower() in ("high", "высокий") else "Проверено")
                ),
                "url": str(scan.get("url", scan.get("URL", ""))),
                "parameter": str(scan.get("parameter", scan.get("Параметр", scan.get("request_params", "")))),
                "severity": severity,
        ...
        for raw_result in results:
            ...
            if vuln_type not in vuln_summary:
                vuln_summary[vuln_type] = {"total": 0, "vulnerable": 0, "safe": 0, "details": []}
...
def export_to_json(data: list[dict[str, Any]], filename: str = "report.json", user_id: int | None = None) -> bool:
...
def export_to_csv(data: list[dict[str, Any]], filename: str = "report.csv", user_id: int | None = None) -> bool:
...
class PDFReport(FPDF):
    ...
    def _set_font(self, size: int) -> None:
        ...
        except (RuntimeError, KeyError, AttributeError):  # fmt: skip
            self._font_name = "Helvetica"
...
def export_to_pdf(data: list[dict[str, Any]], filename: str = "report.pdf", user_id: int | None = None) -> bool:
        ...
        if not data:
            ...
            for i, scan in enumerate(formatted_data, 1):
                ...
                if scan["vulnerabilities"]:
                    ...
                    for vuln_type, vuln_data in scan["vulnerabilities"].items():
                        pdf.add_text(
                            f"  • {vuln_type}: {vuln_data['vulnerable']} уязвимостей из {vuln_data['total']} проверок"
                        )
...
def export_to_html(data: list[dict[str, Any]], filename: str = "report.html", user_id: int | None = None) -> bool:
...
def export_to_txt(data: list[dict[str, Any]], filename: str = "report.txt", user_id: int | None = None) -> bool:
    ...
    try:
        ...
        report_lines.extend(["=" * 80, "Отчет завершен", "=" * 80])
...
def generate_detailed_report(
    data: list[dict[str, Any]], format_type: str = "json", filename: str | None = None
) -> bool:
...
def export_data(
    data: list[dict[str, Any]], export_type: str, filename: str | None = None, user_id: int | None = None
) -> bool:
...
def export_single_scan_to_json(
    scan: dict[str, Any], filename: str = "single_scan_report.json", user_id: int | None = None
) -> bool:
...
def export_single_scan_to_csv(
    scan: dict[str, Any], filename: str = "single_scan_report.csv", user_id: int | None = None
) -> bool:
...
def export_single_scan_to_pdf(
    scan: dict[str, Any], filename: str = "single_scan_report.pdf", user_id: int | None = None
) -> bool:
    ...
    try:
        ...
        if results:
            ...
            for i, result in enumerate(results, 1):
                pdf.add_text(
                    f"{i}. {result.get('type', 'Unknown')} - {result.get('url', 'Unknown')}", 10, url_mode=True
                )
...
def export_single_scan_to_html(
    scan: dict[str, Any], filename: str = "single_scan_report.html", user_id: int | None = None
) -> bool:
...
def export_single_scan_to_txt(
    scan: dict[str, Any], filename: str = "single_scan_report.txt", user_id: int | None = None
) -> bool:
...
def export_single_scan(
    scan: dict[str, Any], export_type: str, filename: str | None = None, user_id: int | None = None
) -> bool:
