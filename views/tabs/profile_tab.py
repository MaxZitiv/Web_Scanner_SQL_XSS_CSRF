...
from PyQt6.QtWidgets import QFileDialog, QFormLayout, QGroupBox, QHBoxLayout, QLabel, QPushButton, QVBoxLayout, QWidget
...
class ProfileTabWidget(QWidget):
    ...
    def load_user_data(self):
        try:
            user_data: dict[str, Any] | None = db.get_user_by_id(self.user_id)
            if user_data:
    ...
    def load_scan_statistics(self):
        try:
            ...
            if scans:
                ...
                total_vulns = 0
                for scan in scans:
                    results: list[Any] | str = scan.get("result", scan.get("results", []))
    ...
    def change_avatar(self):
        try:
            file_path, _ = QFileDialog.getOpenFileName(self, "Выберите аватар", "", "Images (*.png *.jpg *.jpeg *.bmp)")
            if file_path:
                ...
                avatar_name = (
                    f"avatar_{get_local_timestamp().replace(':', '').replace(' ', '_')}{os.path.splitext(file_path)[1]}"
                )
                ...
                with db.get_db_connection_cm() as conn:
                    conn.execute("UPDATE users SET avatar_path = ? WHERE id = ?", (avatar_path, self.user_id))
    ...
    def remove_avatar(self):
        try:
            ...
            if user_data:
                ...
                # Update database using get_db_connection_cm
                with db.get_db_connection_cm() as conn:
                    conn.execute("UPDATE users SET avatar_path = NULL WHERE id = ?", (self.user_id,))
    ...
    def set_avatar(self, pixmap: QPixmap):
        ...
        try:
            ...
            logger.exception(f"Error setting avatar: {e!s}")
    ...
    def set_default_avatar(self):
        ...
        try:
            ...
            logger.exception(f"Error setting default avatar: {e!s}")
