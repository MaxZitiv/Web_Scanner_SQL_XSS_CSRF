...
class StatsTabWidget(QWidget):
    ...
    def _create_filter_panel(self) -> QWidget:
        ...
        cast(Any, self.period_combo).addItems(
            ["За все время", "За последний месяц", "За последнюю неделю", "За сегодня", "Произвольный период"]
        )
        ...
        cast(Any, self.scan_type_combo).addItems(
            ["Все типы", "Быстрое сканирование", "Полное сканирование", "Только SQL", "Только XSS", "Только CSRF"]
        )
    ...
    def _apply_filters(self, scans: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        if scan_type != "Все типы":
            ...
            filtered_scans = [scan for scan in filtered_scans if scan.get("scan_type", "") == filtered_type]
    ...
    def _refresh_stats_with_matplotlib(self, scans: list[dict[str, Any]]):
        ...
        try:
            if not scans:
                ...
                _ = cast(Any, ax).text(
                    0.5, 0.5, "Нет данных для отображения", horizontalalignment="center", verticalalignment="center"
                )
                ...
                if isinstance(results, list):
                    for result in cast(list[Any], results):
                        ...
                        if isinstance(result, dict):
                            ...
                            type_value: str | None = cast(str | None, result_dict.get("type"))
                            vuln_type_value: str | None = cast(str | None, result_dict.get("vuln_type"))
            ...
            for vuln_type in vulnerability_counts:
                # Явно указываем типы для Pylance
                counts: list[int] = []
            ...
            cast(Any, ax).text(
                0.5,
                0.5,
                f"Ошибка отображения статистики: {e!s}",
    ...
    def _export_to_png(self):
        ...
        try:
            file_path, _ = QFileDialog.getSaveFileName(self, "Сохранить график", "", "PNG Files (*.png)")
            if file_path:
    ...
    def _export_to_csv(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(self, "Сохранить статистику", "", "CSV Files (*.csv)")
            if file_path:
