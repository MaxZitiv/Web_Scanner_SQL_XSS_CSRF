...
class DashboardWindow(QMainWindow):
    ...
    async def on_start_scan(self):
        ...
        try:
            ...
            if not scan_types:
                error_handler.show_error_message(
                    "Ошибка", "Выберите хотя бы один тип сканирования:• SQL Injection\n• XSS\n• CSRF"
                )
    ...
    def on_stop_scan(self):
        ...
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes and self.scan_controller:
    ...
    def connect_scan_signals(self):
        ...
        try:
            ...
            if self.statistics_widget is not None:
                ...
                def progress_handler(progress: Any) -> None:
                    return self.update_progress_in_main_thread(progress)
    ...
    def on_logout(self):
        try:
            ...
            error_handler.show_error_message("Ошибка", f"Ошибка при выходе: {e!s}")
