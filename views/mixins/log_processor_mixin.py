...
from PyQt6.QtWidgets import QLabel, QTableWidget, QTextEdit
...
class LogProcessorMixin:
    ...
    def __init__(self):
        ...
        self.detailed_log: QTextEdit | None = None
        # Метка для отображения статуса лога
        self.log_status_label: QLabel | None = None
        # Таблица последних сканирований
        self.recent_scans_table: QTableWidget | None = None
        # Список записей лога
        ...
        try:
            ...
            try:
                ...
                scan_results: list[dict[str, Any]] = [
                    {"timestamp": get_local_timestamp(), "message": line.strip()} for line in lines if line.strip()
                ]

                # Отправка сигнала с результатами
                self._scan_result_signal.emit({"results": scan_results, "url": "Unknown", "scan_type": "Unknown"})
    ...
    def _update_log_display(self):
        ...
        try:
            if hasattr(self, "detailed_log") and self.detailed_log is not None:
                ...
                entries_to_display = self._filtered_log_entries or self._log_entries

                # Форматирование и отображение записей
                for entry in entries_to_display:
                    formatted_entry = f"[{entry['timestamp']}] {entry['level']}\n{entry['message']}\n{'=' * 80}\n"
