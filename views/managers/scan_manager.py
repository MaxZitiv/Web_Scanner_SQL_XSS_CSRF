...
class ScanManagerStatsMixin:
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    and hasattr(dashboard, "stats_labels")
                    and (stats_labels := getattr(dashboard, "stats_labels", None)) is not None
                    and hasattr(stats_labels, "__getitem__")
                    and key in stats_labels
                ):
        ...
        if hasattr(dashboard, "scan_controller") and dashboard.scan_controller is not None:
            try:
                ...
                if hasattr(dashboard, "scan_status"):
                    dashboard.scan_status.setText(f"Ошибка: {e!s}")
    ...
    def _update_scan_time(self):
        ...
        if (
            not hasattr(dashboard, "_scan_timer")
            or dashboard._scan_timer is None
            or not dashboard._scan_timer.isActive()
        ):
            return

        # Проверяем, что статистика существует и имеет нужную структуру
        if not hasattr(dashboard, "_stats") or dashboard._stats is None:
            dashboard._stats = cast(
                dict[str, Any],
                {
                    "urls_found": 0,
                    "urls_scanned": 0,
